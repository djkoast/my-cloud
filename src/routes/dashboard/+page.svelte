...
<div class="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
          ...
          <div class="flex items-center justify-between p-3 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
            <div class="flex items-center gap-3 flex-1 min-w-0">
              ...
              {:else}
                {#if file.mime_type.startsWith('image/') && thumbnails[file.id]}
                  <img src={thumbnails[file.id]} alt={file.name} class="w-10 h-10 object-cover rounded cursor-pointer" onclick={() => openPreview(file)} />
                {:else}
                  <span class="text-xl cursor-pointer" onclick={() => openPreview(file)}>
                    {file.mime_type.startsWith('video/') ? '🎬' : file.mime_type === 'application/pdf' ? '📄' : '📎'}
                  </span>
                {/if}
                <div class="min-w-0">
                  {#if renameFileId === file.id}
            ...
            <div class="flex gap-2">
              {#if tab === 'myfiles'}
                ...
                {#if file.mime_type !== 'folder'}
                  <button onclick={() => showVersions(file.id, file.name)} class="text-sm text-purple-500 hover:underline">Versions</button>
                {/if}
              {/if}
  ...
  {#if showVersionsModal}
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onclick={() => showVersionsModal = false}>
      <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg w-full max-w-md max-h-[80vh] overflow-auto" onclick={(e) => e.stopPropagation()}>
        <h2 class="text-lg font-bold mb-4">Versions of {versionsFileName}</h2>
        {#if versionsList.length === 0}
          <p class="text-gray-500 dark:text-gray-400">No previous versions.</p>
        {:else}
          <ul class="space-y-2">
            {#each versionsList as version}
              <li class="flex justify-between items-center border-b dark:border-gray-700 pb-2">
                <div>
                  ...
                  <span class="text-xs text-gray-500 ml-2">{new Date(version.created_at).toLocaleDateString()}</span>
                  <span class="text-xs ml-2">{version.size ? (version.size / 1024).toFixed(1) + ' KB' : ''}</span>
                </div>
                <button onclick={() => downloadVersion(version.storage_path)} class="text-sm text-blue-500 hover:underline">Download</button>
              </li>
            {/each}
          </ul>
        {/if}
        <div class="mt-4 text-right">
          <button onclick={() => showVersionsModal = false} class="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded">Close</button>
        </div>
      </div>
    </div>
  {/if}
